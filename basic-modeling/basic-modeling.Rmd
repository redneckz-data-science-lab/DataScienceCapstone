...
(function() {
    ...
    group.count <- 5 # Memory/performance trade-off
...
```{r}
    CombineNgramFreqTablesGroups <- function(files.groups) {
        ngram.freq.table <- ComputeNgramFreqTablesGroup(files.groups[[1]])
...
if (!file.exists("./output/test.freq.table.rds")) {
    ...
    test.files.group <- sapply(ngram.freq.files.groups,
                               function(files.group) sample(files.group, 1))
    test.freq.table <- ComputeNgramFreqTablesGroup(test.files.group)
    test.freq.table <- test.freq.table[n > 1, .SD[which.max(freq)],
                                       by=first.words]
...
TestPredictorAgainstQuiz <- function(predictor, train.freq.table,
         test.quiz,
         max.ngram.count=10) {
    return(sapply(names(test.quiz), function(query) {
        expected.next.words <- test.quiz[[query]]
        next.words <- predictor(ngram.freq=train.freq.table,
                                query=query,
                                max.ngram.count=max.ngram.count)
        return(intersect(expected.next.words, next.words))
    }))
}
```

```{r}
TestPredictorAgainstQuiz(PredictNextWords,
                         train.freq.table,
                         test.quiz.1)
```
...
```{r}
SmoothNgramFreqTableByKneserNey(train.freq.table)
```
