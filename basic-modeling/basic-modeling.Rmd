...
set.seed(123456)
ngram.freq.files.index <- rbinom(length(ngram.freq.files),
                                 size=1, prob=0.3) == 1
sampled.ngram.freq.files <- ngram.freq.files[ngram.freq.files.index]
cv.ngram.freq.files <- split(sampled.ngram.freq.files,
        factor(rbinom(length(sampled.ngram.freq.files),
                      size=1, prob=0.1), levels=c(0, 1)))
train.freq.table <- CombinePartitions(cv.ngram.freq.files[[1]],
                                      reduce.f=MergeNGramFreqTables)
test.freq.table <- CombinePartitions(cv.ngram.freq.files[[2]],
                                     reduce.f=MergeNGramFreqTables)
summary(train.freq.table[order(-freq)])
```
...
```{r}
train.freq.table <- train.freq.table[order(-freq)]
```
...
EstimatePredictorAccuracy <- function(predictor,
                                      ...
                                      test.freq.table,
                                      max.ngram.count=10) {
    most.freq.table <- test.freq.table[n > 1, .SD[which.max(freq)],
                                       by=first.words]
    ...
    assertion.result <- most.freq.table[, assert(first.words,
                                                 expected.word=last.word),
                                        by=.I]
