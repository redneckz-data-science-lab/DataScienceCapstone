(function() {
    ...
    MergeNGramFreqTables <<- function(x, y) {
        ...
        res[is.na(freq.x), freq.x := 0]
        res[is.na(freq.y), freq.y := 0]
    ...
    ToNGramFreqTable <<- function(term.doc.matrix) {
        iv <- data.table(i=term.doc.matrix$i, v=term.doc.matrix$v)
        ...
        ngram.freq[, first.words := ifelse(is.na(index), "",
                                           stri_sub(terms, to=index - 1))]
        ngram.freq[, last.word := ifelse(is.na(index), terms,
                                         stri_sub(terms, from=index + 1))]
