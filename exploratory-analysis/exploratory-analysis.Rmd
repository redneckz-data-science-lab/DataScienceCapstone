...
ReadAndCleanCorpus <- function(dir, prob) {
    ...
    for (file.path in files) {
        sentences <- ReadSomeSentences(file.path, sum(prob))
        current.corpus <- CleanCorpus(VCorpus(VectorSource(sentences)))
        if (is.null(corpus)) {
            corpus <- current.corpus
        } else {
            corpus <- c(corpus, current.corpus)
        }
    }
    return(corpus)
}

ReadSomeSentences <- function(file.path, prob) {
    lines <- readLines(file.path, warn=F, skipNul=T)
    sampled.lines <- lines[rbinom(n=length(lines), size=1,
                                  prob=sum(prob)) == 1]
    annotator <- Maxent_Sent_Token_Annotator()
    sampled.sentences <- Map(f=function(line) {
        str <- as.String(line)
        return(str[annotate(str, annotator)])
    }, sampled.lines)
    return(Reduce(f=c, x=sampled.sentences, init=character()))
}
```
